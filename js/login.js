...
$(function () {
  ...
  if (youtubeId != undefined) {
    $('#ytube_link').attr('src', `https://www.youtube.com/embed/${youtubeId}?autoplay=1&controls=0&mute=1&modestbranding=1&rel=0&${youtubeList}&loop=1`);
  } else {
    res.youtubeRemove();
  }
  ...
  res = {
    ...
    invite: function () {
      ...
      $('#wrap > section > div > div.invite-btn > a')
        .off('click')
        .on('click', function () {
          let input = $('#wrap > section > div > div.invite-popup-wrap > div.url-receive-user > form > input[type=text]');
          if ($('.invite-popup-wrap').is(':visible')) {
            $('.invite-popup-wrap').hide('drop');
            input.val('');
          } else {
            $('#wrap > section > div > div.invite-popup-wrap > div.url-copy > form > input[type=text]').val(window.location.href);
    ...
    roomInitTagRemove: function (sub) {
      if (sub) {
        ...
        $('div.newchat-comment-wrap, div.entery, div.chatout, div.notice, div.whisper, div.content', $('.newchat-chat-contents')).remove();
      } else {
